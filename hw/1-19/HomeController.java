...
public class HomeController {
	...
	//겟으로 /messageInput 왔을 때 messege.jsp로 가주세요.
	@RequestMapping(value = "/messageInput", method = RequestMethod.GET)
	public String messageInput(Locale locale, Model model, String description, String replyId) {
		...
		mysql.getConnection();
		mysql.insertReply(description, replyId);
	...
	//겟으로 /message에 왔을 때 messege.jsp로 가주세요.
	...
	public static String likeInstaIdCal(ArrayList<Like> likeList) {
		...
		//좋아요가 하나도 없어서 에러가 날 수 있으니 이걸 방지하는 방법 1
		//if(likeList.size()==0) {
		//	pick.setInstaId("좋아하는 사람이 없음");
		//}else {
		//	pick = likeList.get( random.nextInt(likeList.size()) );
		//}

			
		//좋아요가 하나도 없어서 에러가 날 수 있으니 이걸 방지하는 방법 2 
		try{
			pick = likeList.get( random.nextInt(likeList.size()) );
		}catch(Exception e) {
			pick.setInstaId("좋아하는 사람이 없음");
