...
public class HomeController {
	...
	//겟으로 /postAll에  왔을때 postAll.jsp로 가주세요.
	@RequestMapping(value = "/postAll", method = RequestMethod.GET)
	public String postAllPage(Locale locale, Model model) {
	...
	//겟으로 /replyForm에  왔을때 replyForm.jsp로 가주세요.
			@RequestMapping(value = "/replyFormAction", method = RequestMethod.POST)
			public RedirectView replyFormAction(Locale locale, Model model, Long postId, String instaId, String description) {
				
				MySql mysql = new MySql();
				mysql.getConnection();
				mysql.insertReply(postId, instaId, description );
				System.out.println(description);
				return new RedirectView("replyForm");
				
			}
			@RequestMapping(value = "/replyForm", method = RequestMethod.GET)
			public String replyForm(Locale locale, Model model) {
				return "replyForm";
			}
	
	//겟으로 /postForm이 왔을때 postForm.jsp로 가주세요.
		@RequestMapping(value = "/postFormAction", method = RequestMethod.POST)
		public RedirectView postFormAction(Locale locale, Model model, String picture, String postId, String description) {
		
			MySql mysql = new MySql();
			mysql.getConnection();
			mysql.insertPost(picture, postId, description);
	...
	//겟으로 /post에  왔을때 post.jsp로 가주세요.
		@RequestMapping(value = "/post", method = RequestMethod.GET)
		public String postPage(Locale locale, Model model, String postId) {
			
			Post postPage = new Post();
			postPage.setPostId(postId);
		...
		//겟으로 /member에 왔을때 member.jsp로 가주세요.
		...
		public String member(Locale locale, Model model, String instaId) {
			...
			result = mysql.setDataMember(instaId);
	...
	public RedirectView loginInput(Locale locale, Model model, String instaId, String name, String passWord, String avatar) {
		...
		mysql.getConnection();
		//디비에 유저정보 넣기
		mysql.insertLogin(instaId, name, passWord, avatar);
	...
	//로그인 체크 액션
		...
		public RedirectView loginAction(Locale locale, Model model, String instaId, String passWord) {
			...
			success = mysql.idPassCheck(user);
	...
	//겟으로 /login2Page에 왔을때 login2Page.jsp로 가주세요
	@RequestMapping(value = "/login2", method = RequestMethod.GET)
	public String login2Page(Locale locale, Model model) {
	...
	public String messageInput(Locale locale, Model model, String description, String instaId) {
		...
		mysql.getConnection();
		//디비에 리플라이 넣기
		mysql.insertReply(description, instaId);
	...
	//겟으로 /inputPage에 왔을때 inputPage.jsp로 가주세요
	...
	public String home(Locale locale, Model model) {
		...
		mysql.getConnection();
		ArrayList<Post>postList = mysql.getPostList();
		ArrayList<User>guests = mysql.getUserList();
		ArrayList<Reply>replyList = mysql.getReplyList2();
		
		for(Post post : postList) {
			ArrayList<Reply> tempList = new ArrayList<Reply>();
			for(Reply reply : replyList) {
				
				if(post.getPostNumber() == reply.getPostId()) {
					tempList.add(reply);
				}
			}
...
//		guests.add(guest5);
		...
		/*post1.setLikeInstaId(likeInstaIdCal(resultLikeList));
		post2.setLikeInstaId(likeInstaIdCal(resultLikeList));
		post3.setLikeInstaId(likeInstaIdCal(resultlikeList));*/
