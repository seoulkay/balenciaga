function matching(user){
	    chrome.tabs.executeScript({code:'document.querySelector("body").innerText'
	},  function(result) {
		...
		//bodyText에서 자신이 알고 있는 단어(the)가 몇법 등장하는지를 알아본다. 그 결과를 myNum이라는 변수에 담는다.
		var myNum = bodyText.match(new RegExp('\\b(' + user + ')\\b','gi')).length; 
		
		var per = myNum / bodyNum * 100 ;
		per = per.toFixed(1); //결과값의 소수점을 고정시킨다. ex)두자리까지
...
//컨텐트 페이지의 #user입력된 값이 변경되었을 '때'
document.querySelector('#user')
.addEventListener('change', function(){	
	//컨텐트 페이지에 몇개의 단어가 등장하는지 계산해주세요.
